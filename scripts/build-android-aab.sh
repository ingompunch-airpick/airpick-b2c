...
if ! command -v java >/dev/null 2>&1; then
  echo "❌ Java(JDK)가 없습니다."
  echo "   Android Studio 설치 후 터미널을 재시작하거나,"
  echo "   JDK 17+ 를 설치하세요: https://developer.android.com/studio"
  exit 1
fi

if [[ -z "${ANDROID_HOME:-}" && ! -d "$HOME/Library/Android/sdk" ]]; then
  echo "❌ Android SDK 가 없습니다."
  echo "   Android Studio → Settings → Android SDK 설치 후"
  echo "   ~/.zshrc 에 추가:"
...
KEYSTORE_FILE="$ROOT_DIR/android/airpick-release.keystore"
...
if [[ ! -f "$KEYSTORE_PROPS" ]]; then
  ...
  if [[ ! -f "$KEYSTORE_FILE" ]]; then
    ...
    read -r -p "키 별칭 [airpick]: " KEY_ALIAS
    KEY_ALIAS="${KEY_ALIAS:-airpick}"
    keytool -genkeypair -v \
      -keystore "$KEYSTORE_FILE" \
      -alias "$KEY_ALIAS" \
      -keyalg RSA -keysize 2048 -validity 10000 \
      -storepass "$STORE_PASS" -keypass "$STORE_PASS" \
      -dname "CN=AirPick, OU=Mobile, O=AirPick, L=Seoul, ST=Seoul, C=KR"
  fi
  cat > "$KEYSTORE_PROPS" <<EOF
storeFile=../airpick-release.keystore
