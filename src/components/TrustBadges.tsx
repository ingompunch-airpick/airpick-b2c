  ...
  return (
    <div className={cn('flex flex-wrap gap-1.5', size === 'md' && 'gap-2')}>
      {badgeConfig.map(({ key, icon: Icon, label }) => {
        if (key === 'location' && company.sharesParkingLocation === false) return null;
        if (key === 'photos' && company.sharesPhotos === false) return null;
        if (key === 'insurance' && !insuranceLabel) return null;

        const text =
          key === 'insurance' && insuranceLabel !== '보험'
            ? insuranceLabel
            : label;
