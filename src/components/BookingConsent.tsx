import { PRIVACY_CONSENT, TERMS_OF_SERVICE } from '../constants/consent';
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="rounded-2xl bg-sky-bg p-3 ring-1 ring-sky-border/70">
        <p className="text-[11px] font-bold text-ink">{TERMS_OF_SERVICE.title}</p>
        <div className="mt-2 max-h-24 overflow-y-auto rounded-xl bg-sky-soft p-2.5 text-[10px] leading-relaxed text-muted">
          {TERMS_OF_SERVICE.articles.map(({ heading, body }) => (
        ...
        <label className="mt-2 flex items-start gap-2">
          ...
          />
          <span className="text-[11px] font-semibold text-ink">{TERMS_OF_SERVICE.checkbox}</span>
        </label>
      </div>
