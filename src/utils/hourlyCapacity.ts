/** B2B `hourlyCapacity.ts`와 동일 규칙 — 시간당 입고 한도 */
...
function normalizeDateYmd(raw: string | undefined | null): string {
  if (!raw) return '';
...
export function reservationInHourBucket(
  ...
  const dep = normalizeDateYmd(res.departureDate);
  const target = normalizeDateYmd(departureDate);
