...
export default defineConfig({
    ...
    VitePWA({
      ...
      workbox: {
        ...
        runtimeCaching: [
          ...
          {
            urlPattern: /\/(?:about|faq|privacy|guides|partners|for-partners|facts)(?:\/|$)/i,
  ...
  server: {
    ...
    proxy: {
      ...
      '/api/icn-flight-search': {
        target: 'https://asia-northeast3-airpick-reservation.cloudfunctions.net',
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/api\/icn-flight-search/, '/getIcnFlightSearch'),
      },
      '/api/icn-flight': {
        target: 'https://asia-northeast3-airpick-reservation.cloudfunctions.net',
        changeOrigin: true,
